...
public class ZooKeeper implements AutoCloseable {
     ...
     * 在收到服务端的响应后，会从packet中的watchRegistration中拿出对应的watcher 并注册到ZKWatchManager（转交wath）
     * 客户端维护存储其会话期内注册在服务器的watcher
    ...
    public abstract class WatchRegistration {
         ...
         * 客户端会将之前暂时保存的watcher对象转交给zkwatchManager，最终保存到dataWatches
         */
     ...
     *            a watcher object which will be notified of state changes, may
     *            also be notified for node events
     * @param canBeReadOnly 当前机器和集群断联，但仍旧希望它能提供读服务
     *            (added in 3.4) whether the created client is allowed to go to
     *            read-only mode in case of partitioning. Read-only mode
     *            basically means that if the client can't find any majority
     *            servers but there's partitioned server it could reach, it
     ...
     *            session timeout in milliseconds
     * @param watcher
     *            是客户端会话期间的默认watcher，保存在defaulWatcher中
     *            a watcher object which will be notified of state changes, may
     *            also be notified for node events
     * @param sessionId
     *            specific session id to use if reconnecting
     * @param sessionPasswd
     *            password for this session  除了确定唯一会话外还可以达到会话复用功能（todo 会话复用）
     * @param canBeReadOnly
     *            (added in 3.4) whether the created client is allowed to go to
     *            read-only mode in case of partitioning. Read-only mode
     *            basically means that if the client can't find any majority
    ...
    public byte[] getData(final String path, Watcher watcher, Stat stat)
        throws KeeperException, InterruptedException
     {
        ...
        // the watch contains the un-chroot path
         // 封装watcher对象 保存节点路径和watcher的对应关系
        WatchRegistration wcb = null;
        if (watcher != null) {
        ...
        request.setPath(serverPath);
        //标记request为 使用watcher监听 但不发送watcher对象
        request.setWatch(watcher != null);
